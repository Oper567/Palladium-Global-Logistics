...
const HeroSection = async () => {
  ...
  // Zero layout shift: The server knows the auth state before the HTML even reaches the browser.
    ...
    <section className="relative min-h-[95dvh] flex flex-col items-center justify-center overflow-hidden bg-brand-primary w-full">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none select-none">
        <Image
          ...
          fill
          priority // Tells Next.js to preload this for LCP (Largest Contentful Paint)
          quality={90} // Leverages Next.js automatic WebP compression
      ...
      <div className="relative z-20 container mx-auto px-6 flex flex-col items-center text-center mt-16 md:mt-0">
        ...
        {/* Badge */}
        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mb-8 shadow-2xl" role="status">
        ...
        {/* Headline */}
        <h1 className="text-5xl sm:text-6xl lg:text-8xl font-black text-white uppercase tracking-tighter leading-[0.9] mb-8 drop-shadow-2xl">
          Fast. Reliable. <br />
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 w-full sm:w-auto">
          ...
          <a
            ...
            className="group w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-5 bg-[#25D366] text-white font-black uppercase tracking-widest text-xs md:text-sm rounded-sm hover:bg-[#128C7E] transition-all duration-500 shadow-xl hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-[#25D366] focus:ring-offset-2 focus:ring-offset-brand-primary"
          >
          ...
          {!isSignedIn ? (
            <Link
              href="/sign-up"
              className="w-full sm:w-auto px-10 py-5 bg-white text-brand-primary font-black uppercase tracking-widest text-xs md:text-sm rounded-sm hover:bg-brand-accent transition-all duration-500 shadow-xl hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-brand-primary"
            >
              Open Account
            </Link>
          ) : (
        ...
        <ul className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 border-t border-white/10 pt-10 w-full" aria-label="Company Statistics">
          {fleetStats.map((stat) => (
            // 🚨 React FIX: Using a unique string instead of 'i' prevents DOM mapping bugs
